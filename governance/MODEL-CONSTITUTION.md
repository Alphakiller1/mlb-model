...
| 7 | **Multiple-hypothesis correction** | Any scan over multiple segments applies FDR (BH) for discovery AND Deflated-Sharpe / PBO for strategy selection across trials. | TEST + GATE (selection.py) | ◐ (BH ✅, DSR/PBO ○→added) |
...
| 10 | **Backtest & walk-forward** | Edges validated out-of-sample via walk-forward / purged CV, not a single in-sample scan. Report OOS separately from in-sample. | GATE (promotion needs OOS) | ○ |
| 11 | **Market-price & vig handling** | Probabilities de-vigged two-sided; value measured net of vig at the executable entry price; CLV captured (prob + ROI space) on all analyzed games. | TEST (devig sums to 1) + SCHEMA | ◐ (devig ✅, CLV-all ○) |
| 12 | **Promotion & rollback** | A model/signal reaches production only via champion-challenger on its market's gate. Every promotion is reversible; rollback procedure recorded. | REGISTRY + GATE + HUMAN | ○ |
