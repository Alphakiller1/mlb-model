...
def _logo(abbr, cls="tlogo"):
    ...
    return (
        f'<img class="{cls}" src="https://www.mlbstatic.com/team-logos/{i}.svg" '
...
def _headshot(pid):
    ...
    return (
        f'<img class=phead loading="lazy" alt="" '
...
def _advantage(gd, anchors, repo):
    ...
    idx = (
        {str(r["team"]).upper().strip(): r for _, r in tp.iterrows()}
        if tp is not None and "team" in tp.columns
        else {}
    )
    # pooled series for split-based baselines
    plat_s = (
        pd.concat([s for s in (_col(tp, "osi_vs_lhp"), _col(tp, "osi_vs_rhp")) if s is not None])
        if tp is not None
        else None
    )
    woba_s = (
        pd.concat([s for s in (_col(tp, "home_woba"), _col(tp, "away_woba")) if s is not None])
        if tp is not None
        else None
    )
    obr_s = _col(tp, "obr")
    ...
    def row(cat, a_val, h_val, series, lower_better, base_default, impact, unit="", role="context"):
        ...
        ad = (
            round(a_val - base, 2) if isinstance(a_val, (int, float)) and base is not None else None
        )
        hd = (
            round(h_val - base, 2) if isinstance(h_val, (int, float)) and base is not None else None
        )
        rows.append(
            {
                "cat": cat,
    ...
    row(
        "Pressure-adjusted lineup (PALS)",
        gd.away_context.pals,
        gd.home_context.pals,
        pals_s,
        False,
        50.0,
        "Research context",
    ...
    row(
        "Projected offense (projOSI)",
        gd.away_context.proj_osi,
        gd.home_context.proj_osi,
        proj_s,
        False,
        50.0,
        "Research context",
    ...
    row(
        "Recent offense (OSI L7)",
        gd.away_context.osi_l7,
        gd.home_context.osi_l7,
        osi_l7_s,
        False,
        50.0,
        "Research context",
    ...
    row(
        f"Platoon (OSI vs {gd.home_hand}HP/{gd.away_hand}HP)",
        _tv(idx, gd.away, a_plat_col),
        _tv(idx, gd.home, h_plat_col),
        plat_s,
        False,
        50.0,
        "Total · ML",
    ...
    row(
        "Lineup quality (wOBA)",
        _tv(idx, gd.away, "away_woba"),
        _tv(idx, gd.home, "home_woba"),
        woba_s,
        False,
        0.320,
        "Total · ML",
    ...
    row(
        "Bullpen high-leverage (ERA)",
        _tv(idx, gd.away, "bullpen_high_lev_era"),
        _tv(idx, gd.home, "bullpen_high_lev_era"),
        bhl_s,
        True,
        4.00,
        "Late ML · live",
...
def _extras(away, home, gd, probs, anchors, repo):
    ...
    out = {
        "start": None,
        "lineup": "lineups TBD · probables confirmed",
        "arsenal_a": [],
        "arsenal_h": [],
        "scenario": [],
        "f5": None,
    ...
    def arsenal(name):
        ...
        rows = [
            {
                "pitch": str(x.get("pitch_type", "")),
                "pct": _numf(x.get("pitch_pct")),
                "whiff": _numf(x.get("whiff_rate")),
            }
    ...
    *,
    ...
    markets = [
        _market_row(
            ...
            probs,
            anchors,
            board.quote(away, home, "ml", gd.home),
            promotion,
        ),
        _market_row(
            "ml",
            ...
            probs,
            anchors,
            board.quote(away, home, "ml", gd.away),
            promotion,
        ),
        _market_row(
            "total",
            ...
            probs,
            anchors,
            board.quote(away, home, "total", "over", tline),
            promotion,
        ),
        _market_row(
            "total",
            ...
            probs,
            anchors,
            board.quote(away, home, "total", "under", tline),
            promotion,
        ),
    ]
    ...
    markets.append(
        _market_row(
            ...
            probs,
            anchors,
            board.quote(away, home, "runline", runline_team, runline),
            promotion,
        )
    )
    ...
    return {
        ...
        "confidence": conf,
        "markets": markets,
        "factors": _factors(probs),
        "advantage": _advantage(gd, anchors, repo),
        "simulation": simulation,
        "pitchers": pitcher_rows or [],
        "risks": risk_signals(gd),
        "sharp": _sharp_for(gd.game_pk, reader),
        "game_pk": gd.game_pk,
        "promotion": gate,
...
.c-elite{color:var(--metric-strong)}.c-good{color:var(--metric-good)}.c-mid{color:var(--metric-mid)}.c-weak{color:var(--metric-orange)}.c-poor{color:var(--metric-poor)}.c-na{color:var(--muted)}
/* header */
...
border:2px solid var(--text-4);box-shadow:0 5px 14px rgba(var(--rgb-black),.45),0 0 0 2px rgba(var(--rgb-purple-soft),.12)}
...
.vbar{display:flex;align-items:center;gap:14px;padding:12px 16px;border-radius:10px;border:1px solid var(--border-2);font-size:13.5px;background:linear-gradient(180deg,rgba(24,27,38,.90),rgba(13,15,23,.96));box-shadow:inset 0 1px 0 rgba(var(--rgb-white),.045)}
.vbar b{font-family:var(--display);font-weight:800;font-size:15px;letter-spacing:.03em}.vbar span{color:var(--ink2)}
.vbar.pos{border-color:rgba(var(--rgb-green),.3);background:rgba(var(--rgb-green),.10)}.vbar.pos b{color:var(--positive-soft)}
.vbar.warnc{border-color:rgba(var(--rgb-gold),.3);background:rgba(var(--rgb-gold),.10)}.vbar.warnc b{color:var(--gold)}
.vbar.neg{border-color:rgba(var(--rgb-red),.3);background:rgba(var(--rgb-red),.10)}.vbar.neg b{color:var(--negative-soft)}
...
.pill.pos{background:rgba(var(--rgb-green),.13);color:var(--positive-soft);border-color:rgba(var(--rgb-green),.22)}
.pill.warnc{background:rgba(var(--rgb-gold),.12);color:var(--gold);border-color:rgba(var(--rgb-gold),.22)}
.pill.neg{background:rgba(var(--rgb-red),.12);color:var(--negative-soft);border-color:rgba(var(--rgb-red),.22)}
.pill.mut{background:rgba(var(--rgb-white),.06);color:var(--ink2)}
...
.mmbar i.imp{position:absolute;top:-2px;width:2px;height:16px;background:var(--text-bright)}.mmv{flex:0 0 46px;text-align:right;font-weight:700;font-size:12px}
/* arsenal */
...
.advbar .seg{height:8px;border-radius:3px}.advbar .a{background:var(--ca-purple)}.advbar .h{background:var(--teal)}
/* charts */
...
.pitch-name-meta{display:block;font-size:10px;margin-top:2px}.hold-note{font-size:10px}
...
def _priced_row(market, side, line, model_prob, quote, promotion, label):
    ...
    return {
        ...
        "max": assessment.maximum_odds,
        "state": assessment.action,
        "tone": _ACTION_TONE[assessment.action],
        "reason": assessment.reason,
        "books": quote.book_count if quote else 0,
        "market_time": quote.fetched_at if quote else None,
    }
...
def _f5_market_rows(gd, pitcher_rows, board, promotion, fallback_f5):
    ...
    if proj is None:
        ...
        return [
            {
                "label": f"F5 Total Over {f5['line']:g} (≈)",
                "market": "f5_total",
                "side": "over",
                "line": f5["line"],
                "model": f5["over"],
    ...
    posted = [
        q.line
        for q in board.game_quotes(gd.away, gd.home)
    ...
    return [
        _priced_row(
            ...
            line,
            p_over,
            board.quote(gd.away, gd.home, "f5_total", "over", line),
            promotion,
        ...
        _priced_row(
            ...
            line,
            1 - p_over,
            board.quote(gd.away, gd.home, "f5_total", "under", line),
            promotion,
        ...
        _priced_row(
            ...
            None,
            lead_p,
            board.quote(gd.away, gd.home, "f5_ml", lead_team),
            promotion,
            f"F5 ML {lead_team}",
        ),
    ]
...
def _f5_panel(r, gd, esc):
    ...
    if not f5_rows:
        return ""
    total_over = next(
        (m for m in f5_rows if m["market"] == "f5_total" and m["side"] == "over"), None
    )
    ...
    def quote_bit(m, lean_label):
        ...
        if isinstance(m.get("mkt"), int):
            edge = (
                f" <b class={m.get('tone', 'mut')}>{m['edge']:+.1f}pt</b>"
                if m.get("edge") is not None
                else ""
            )
            tail = (
                f"<span class=mut> live {m['mkt']:+d}"
                + (f" ({esc(str(m.get('book')))})" if m.get("book") else "")
                + "</span>"
                + edge
                + f' <span class="pill {m["tone"]}">{esc(m["state"])}</span>'
            )
        else:
    ...
    if total_over is not None:
        ...
        side_row = (
            ...
            if lean == "over"
                ...
                (m for m in f5_rows if m["market"] == "f5_total" and m["side"] == "under"),
                total_over,
            )
        )
    ...
    if proj is not None:
        ...
        def sp_cell(sp, dist, team):
            ...
            return (
                f"<span><b>{esc(team)} SP · {name}</b>{dist['mean']:.2f} ER F5 "
                f"<span class=mut>({dist['p10']:.0f}–{dist['p90']:.0f})</span></span>"
            )

        sp_html = (
            f"<div class=availability>{sp_cell(proj['away_sp'], proj['away_f5'], gd.away)}"
            f"{sp_cell(proj['home_sp'], proj['home_f5'], gd.home)}</div>"
        )
    priced = any(isinstance(m.get("mkt"), int) for m in f5_rows)
    note = (
        "F5 isolates the starters from the bullpens. Live F5 odds are de-vigged and graded "
        "against the model — same as every other market."
        if priced
        else "F5 isolates the starters from the bullpens. No live F5 price in the feed right now — "
...
def report_body(r):
    ...
    def market_row(market):
        market_price = f"{market['mkt']:+d}" if market["mkt"] is not None else "—"
        ...
        if novig_fair is not None:
            hold_tag = (
                f'<span class="mut hold-note"> {hold:.1f}% hold</span>' if hold is not None else ""
            )
            novig_cell = f"{novig_fair:+d}{hold_tag}"
        else:
            novig_cell = "<span class=mut>—</span>"
        edge = f"{market['edge']:+.1f}pt" if market["edge"] is not None else "—"
        ev = f"{market['ev'] * 100:+.1f}%" if market["ev"] is not None else "—"
        tone = (
            "pos"
            if (market.get("edge") or 0) > 0
            else ("neg" if market.get("edge") is not None else "mut")
        )
        return (
    ...
    umpire_label = umpire.get("umpire") if umpire.get("status") == "announced" else "Not announced"
    ...
    missing = (
        "".join(
            f'<span class="pill warnc">{esc(item)}</span>' for item in probability.missing_context
        )
    ...
    def team_value_rows():
        ...
        def travel_value(value):
            ...
            return f"{value.get('rest_hours', 0):.1f}h rest · {value.get('travel_miles', 0):.0f} mi"
    ...
    def pitcher_panel(team):
        ...
        state_tone = (
            "neg"
            if pitcher["state"] == "REGRESSION"
            else (
                "pos"
                if pitcher["state"] == "PROGRESSION"
                else ("warnc" if pitcher["state"] == "LIMITED SAMPLE" else "side")
            )
        )
    ...
    freshness = f"{fresh_hours:.1f}h old" if fresh_hours is not None else "timestamp unavailable"
