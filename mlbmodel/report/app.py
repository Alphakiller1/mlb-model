...
def _today(slate, sd, sharp_by_pk, sync=None):
    ...
    for g in slate:
        ...
        sharp = (
            ...
            if sc else '<span class=slate-metric><b>0</b><i>sharp</i></span>'
        )
...
def _props(pitchers, prop_board):
    ...
    for index, row in enumerate(pitchers):
        ...
        market_copy = (
            ...
            f'({e(best["best_book"])})'
            if best else "No paired market line"
        )
...
def _results(reader):
    ...
    sharp = reader.get(
        "sharp_observations?settled=eq.true&select=won,push,market_type&limit=1000"
    )
    ...
    verdict_rows = "".join(
        ...
        for label, count in sorted(verdict_counts.items(), key=lambda item: (-item[1], item[0]))[:5]
    ...
    for row in mp_rows[:8]:
        ...
        state = (
            "PUSH" if row.get("push") else
            "WIN" if row.get("won") is True else
            "LOSS" if row.get("won") is False else
            "PENDING"
        )
        tone = "pos" if state == "WIN" else "neg" if state == "LOSS" else "warnc" if state == "PUSH" else "mut"
        ledger_rows += (
...
.navb.on .navmark{color:#fff;background:linear-gradient(135deg,#7C4DFF,#5B2BE0);border-color:transparent}
#main{flex:1;min-width:0;overflow:auto;padding:24px 28px 72px}
.view{display:none}.view.on{display:block}
#main>.view>h2:first-child{font-family:var(--display);font-weight:900;font-size:30px;line-height:1;margin:0 0 4px;background:linear-gradient(180deg,#fff 0%,#E9EAF0 42%,#A4A8B6 64%,#fff 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}
...
.slate-metric--lean b{color:var(--side)}.slate-metric--sharp{border-color:rgba(245,177,76,.35);background:rgba(245,177,76,.08)}.slate-metric--sharp b{color:var(--warn)}
...
@media(max-width:760px){.tracker-grid,.research-grid{grid-template-columns:1fr}.progress-hero{align-items:flex-start;flex-direction:column}.progress-hero span{flex:auto}.progress-meter{width:100%}.result-status-grid,.calibration-cards{grid-template-columns:repeat(2,minmax(0,1fr))}.f5-card-list{display:flex;flex-direction:column;gap:8px}.f5-table{display:none}.calibration-list{display:grid;grid-template-columns:1fr 1fr}.calibration-list+.table-scroll{display:none}.gate-card{min-height:128px}.gate-card b{font-size:28px}}
...
def build_app(featured_game, *, fetch=True, data_dir=None):
    ...
    sync_label = "Exact slate" if sync.get("status") == "exact" else (
        "Live fallback" if sync.get("status") == "fallback" else "Untracked"
    )
    ...
    js = ("function show(k,persist=true){const target=document.getElementById('v-'+k);if(!target)return;"
