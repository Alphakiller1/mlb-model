...
def _today(slate, sd, sharp_by_pk, sync=None):
 ...
 <div class=cards>
   ...
   <div class=card><div class=k>Slate</div><div class=v style="font-size:var(--fs-lg)">{e(sd or "—")}</div></div>
   <div class=card><div class=k>With sharp signal</div><div class=v>{nsharp}</div></div>
   <div class=card><div class=k>MLBMA sync</div><div class=v style="font-size:var(--fs-lg)">{e(sync_label)}</div></div>
 </div>
 <div class=cols>
...
def _markets(slate, sharp_by_pk, model_by_pk=None):
    ...
    def row(d):
        ...
        if isinstance(d["price"], int):
            bet = (f'<b>{e(mkt)} {e(d["sel"])}</b> <span class="pill side">{_num(d["price"])}</span>'
                   ...
                   + (f'<div class=mut style="font-size:var(--fs-2xs)">fair {_num(d["fair"])}</div>'
                      if isinstance(d["fair"], int) else ""))
        else:
            bet = f'<b>{e(mkt)} {e(d["sel"])}</b><div class=mut style="font-size:var(--fs-2xs)">no live #</div>'
        ...
        sharp = (f'<b class={_edge_grade(d["div_pts"] / 100)}>+{d["div_pts"]:.1f}pt</b>'
                 f'<div class=mut style="font-size:var(--fs-2xs)">{d["sharp_p"]:.0f}% vs {d["soft_p"]:.0f}% pub</div>'
                 + ('<span class="pill warnc" style="font-size:var(--fs-2xs)">STEAM</span>' if d["steam"] else ""))
        ...
        if d["medge"] is not None:
            ...
            model = (f'<b class={_edge_grade(d["medge"] / 100)}>{d["medge"]:+.1f}pt</b>'
                     f'<div class=mut style="font-size:var(--fs-2xs)">model {d["model_p"]:.0f}% · {ev_txt}</div>')
 ...
 <div class=cards>
   <div class=card><div class=k>Top play</div><div class=v style="font-size:var(--fs-lg)">{e(top_txt)}</div>
     <div class=mut style="font-size:var(--fs-2xs)">{e(top_sub)}</div></div>
   ...
   <div class=card><div class=k>Suggested exposure</div><div class=v>{exposure:.1f}u</div>
     <div class=mut style="font-size:var(--fs-2xs)">{n_pass} pass</div></div>
 </div>
        ...
        for label, board in sources:
            ...
            for key in _PICKEM_ORDER:
                ...
                variant = (
                    ...
                    else f' <span class="mut" style="font-size:var(--fs-2xs)">{e(str(line.get("odds_type")))}</span>'
                )
                count += 1
                rows += (
                    f'<tr>{pitcher_cell}'
...
def _props(pitchers, prop_board, pp_board=None, ud_board=None, sl_board=None):
 ...
 <div class=cards>
   ...
   <div class=card><div class=k>Price feed</div><div class=v style="font-size:var(--fs-lg)">{"LIVE" if all_markets else "NO SNAPSHOT"}</div></div>
 </div>
 ...
 <div class=sec><h2>Pick&apos;em boards <span class="mut" style="font-weight:600;font-size:var(--fs-2xs)">PrizePicks + Underdog + Sleeper · model vs line · {pickem_count} lines</span></h2><div class=body>
...
def _portfolio(reader, gate, slate):
 ...
 <div class=cards>
   ...
   <div class=card><div class=k>Auto sizing</div><div class=v style="font-size:var(--fs-lg)">{sizing_state}</div></div>
 </div>
...
def _trends(reports):
 ...
 <div class=cards>
   ...
   <div class=card><div class=k>Source</div><div class=v style="font-size:var(--fs-lg)">MLBMA logs</div></div>
 </div>
...
/* Section nav lives in the Chase header (chase_nav.css); the old left sidebar was retired. */
#main{max-width:1240px;margin:0 auto;padding:var(--sp-6) var(--sp-6) 72px}
...
#main>.view>h2:first-child{font-family:var(--display);font-weight:800;font-size:var(--fs-2xl);font-variation-settings:'wdth' 125;
letter-spacing:-.02em;margin:0 0 5px;line-height:1.05}
.ctx{color:var(--muted);font-size:var(--fs-sm);margin-bottom:18px}
.note{color:var(--muted);font-size:var(--fs-xs);margin-top:10px}
...
.card .k{color:var(--muted);font-size:var(--fs-2xs);text-transform:uppercase;letter-spacing:.07em;font-weight:800}
.card .v{color:var(--ink);font-family:var(--display);font-weight:800;font-size:var(--fs-2xl);font-variation-settings:'wdth' 120;
...
.pagehead h2{font-family:var(--display);font-weight:800;font-size:var(--fs-2xl);font-variation-settings:'wdth' 125;letter-spacing:-.02em;margin:0 0 5px;line-height:1.05}
.pagehead .ctx{margin:0}
.pagehead select{min-width:180px;background:var(--card);color:var(--ink);border:1px solid var(--border-2);border-radius:10px;padding:10px 13px;font:600 var(--fs-sm) var(--sans);transition:border-color .15s ease}
...
background:linear-gradient(135deg,rgba(124,77,255,.12),rgba(45,212,191,.04));color:var(--ink2);font-size:var(--fs-xs);margin-bottom:18px;overflow:hidden}
...
.empty{color:var(--muted);font-size:var(--fs-sm);padding:18px;border:1px dashed var(--border-2);border-radius:8px}
...
.pitcher-cell>div>span{display:flex;align-items:center;gap:5px;color:var(--muted);font-size:var(--fs-2xs);margin-top:3px}
.starter-base{min-width:88px}.starter-base b{display:block}.starter-base span{display:block;color:var(--muted);font-size:var(--fs-2xs);margin-top:2px;white-space:nowrap}
.prop-cell{min-width:82px}.prop-cell>b{display:block;font:800 var(--fs-lg) var(--display)}
.prop-range{display:block;font-size:var(--fs-2xs);color:var(--muted);white-space:nowrap}.prop-mkt{display:block;font-size:var(--fs-2xs);margin-top:3px;white-space:nowrap}
.prop-sub{display:block;color:var(--muted);font-size:var(--fs-2xs);margin-top:4px}
...
.detail-strip{display:flex;gap:14px;flex-wrap:wrap;color:var(--muted);font-size:var(--fs-2xs);margin:0 0 10px}
.detail-strip b{color:var(--ink)}
.trend-sig{text-align:left;color:var(--ink2);font-size:var(--fs-sm);max-width:560px;white-space:normal;line-height:1.4}
.edge-row{display:flex;gap:18px;margin:2px 0 14px}.edge-cell{flex:1;min-width:0}
.edge-cell .k{color:var(--muted);font-size:var(--fs-2xs);text-transform:uppercase;letter-spacing:.06em;font-weight:800}
...
.edgebar b{position:absolute;right:8px;top:0;line-height:20px;font-family:var(--display);font-weight:800;font-size:var(--fs-xs)}
.trend-list{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:9px}
.trend-list li{font-size:var(--fs-sm);line-height:1.5;color:var(--ink2);padding-left:2px}
.trend-list .pill{margin-right:6px;vertical-align:middle}
@media(max-width:760px){
  #main{padding:var(--sp-4) var(--sp-3) 64px}
  .cards{grid-template-columns:repeat(2,1fr)}
  #main>.view>h2:first-child,.pagehead h2{font-size:var(--fs-2xl)}
}
/* Dense data tables scroll inside their own container (never the page) with a sticky first
   column, so there is no horizontal page overflow at 375px. */
.table-scroll{-webkit-overflow-scrolling:touch}
@media(max-width:520px){.prop-table{min-width:720px}}
"""
